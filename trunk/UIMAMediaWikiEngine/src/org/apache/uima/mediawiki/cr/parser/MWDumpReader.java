...
public class MWDumpReader {
			...
			while (!endPage) {
				switch (MWTag.toTag(reader.getLocalName())) {
					...
					case PAGE:
						...
						break;
					default:
						// This happens if we don't process a tag acknoledged in the MWTag enum.
						endPage = true;
				}
				if (!endPage)
					nextOpeningTag(1);
		...
		while (!endRevision) {
			switch (MWTag.toTag(reader.getLocalName())) {
				...
				case TIMESTAMP:
					...
					break;
				case CONTRIBUTOR:
					// We are only interrested in the user name. This is a nested element of this tag.
					// The user name tag is however optional.
					nextOpeningTag(1);
					if (reader.getLocalName().equals("username"))
						MWRevisionFactory.hasContributor(getTagText());
					break;
				case MINOR:
					// If the tag is <minor /> then it's set to false
					reader.next();
					MWRevisionFactory.isMinor(reader.getEventType() == END_ELEMENT);
					break;
				case COMMENT:
	...
	private void nextTag(String name) throws XMLStreamException, NoSuchElementException {
		while (reader.getEventType() != START_ELEMENT && !reader.getLocalName().equals(name))
			if (reader.hasNext())
				reader.next();
			else
	...
	private final void skipThisTag() throws XMLStreamException {
		...
		while (!endOfTag)
			if (reader.hasNext()) {
				reader.next();
				if (reader.getEventType() == END_ELEMENT && reader.getLocalName().equals(name))
					endOfTag = true;
			} else
				throw new NoSuchElementException("End of document reached");
	}
	...
	private final String getTagText() throws XMLStreamException {
		reader.next();
		return reader.getEventType() == CHARACTERS ? reader.getText() : "";
	}
