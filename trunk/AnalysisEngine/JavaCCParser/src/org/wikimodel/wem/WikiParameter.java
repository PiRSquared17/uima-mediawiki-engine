...
public class WikiParameter {
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof WikiParameter))
			return false;
		final WikiParameter pair = (WikiParameter) obj;
		return fKey.equals(pair.fKey) && (fValue == pair.fValue || fValue != null && fValue.equals(pair.fValue));
	}
	...
	public boolean isValid() {
		if (fValid == null) {
			final int len = fKey != null ? fKey.length() : 0;
			boolean result = len > 0;
			boolean delimiter = false;
			for (int i = 0; result && i < len; i++) {
				final char ch = fKey.charAt(i);
				if (ch == ':') {
