...
/**
 * This class contains some utility methods used for escaping xml strings as well as for encoding/decoding http
 * parameters.
 * 
 * @author kotelnikov
 */
public class WikiPageUtil {
	...
	/**
	 * Returns the decoded http string - all special symbols, replaced by replaced by the %[HEX HEX] sequence, where
	 * [HEX HEX] is the hexadecimal code of the escaped symbol will be restored to its original characters (see RFC-2616
	...
	/**
	 * Returns the encoded string - all special symbols will be replaced by %[HEX HEX] sequence, where [HEX HEX] is the
	 * hexadecimal code of the escaped symbol (see RFC-2616 http://www.w3.org/Protocols/rfc2616/rfc2616.html).
	 * 
	 * @param str
	 ...
	 *            the string to escape
	 * @param escapeQuots
	 *            if this flag is <code>true</code> then "'" and "\"" symbols also will be escaped
	 * @return the escaped string.
	 */
	public static String escapeXmlString(String str, boolean escapeQuots) {
		...
		for (final char element : array) {
			if (element == '>' || element == '&' || element == '<' || escapeQuots && (element == '\'' || element == '"')) {
				buf.append("&#x" + Integer.toHexString(element) + ";");
			} else {
				buf.append(element);
			}
		}
	...
	public static String getCDATA(String text) {
		...
		while (startPos >= 0 && startPos < text.length()) {
			final int id = text.indexOf("]]>", startPos);
			if (id >= 0) {
