...
public class MediawikiScanner implements MediawikiScannerConstants {
	...
	/**
	 * Count number of empty lines to send when we emit the onEmptyLines event. We need to count them since the number
	 * we send depend on the next block element. For all block elements other than paragraph we send one more empty
	 * lines since these blocks are recognized by the lexer as NewLine token followed by the token for the element and
	 * thus that "eats" one NewLine which is why we need to add it again.
	 */
	private int							emptyLinesCount		= 0;

	private final IWikiReferenceParser	fReferenceParser	= new MediaWikiReferenceParser();
	...
	final public void header() throws ParseException {
		...
		t = getHEADER_BEGIN();
		sendOnEmptyLinesEvent(true);
		final int level = t.image.trim().length();
	...
	final public void tableRow() throws ParseException {
		...
		sendOnEmptyLinesEvent(true);
		final String str = t.image.trim();
		final boolean head = str.startsWith("!");
	...
	final public void verbatimBlock() throws ParseException {
		...
		final String tag = str.substring(1, str.indexOf('>'));
		int len = 0;
		int end = 0;
		WikiParameters params = null;
			...
			switch (jj_ntk == -1 ? jj_ntk() : jj_ntk) {
				...
				case D_FORMAT_SYMBOL:
					...
					if (str.startsWith("<")) {
						...
						final boolean close = str.startsWith("/");
						if (close) {
							str = str.substring(1);
						}
						if ("sub".equals(str)) {
							...
							new WikiParameters(str.substring("ref".length() + 1).trim());
							fContext.onFormat(IWikiScannerContext.REF, close);
						}
					} else {
						if ("'''''".equals(str)) {
				...
				case D_REFERENCE:
					...
					str = str.trim();
					final WikiReference ref = fReferenceParser.parse(str);
				...
				case D_TABLE_CELL_NL:
					...
					if (fContext.isInTable()) {
						str = t.image.trim();
						final boolean headnl = str.startsWith("!");
	...
	/**
	 * If shouldIncrement is true send one more empty lines. All block elements other than paragraph should call
	 * shouldIncremet with true since we need to emit one more new line for them (since the lexer "eats" a NewLine token
	 * to recognize these block elements.
	 */
	...
	private Token jj_consume_token(int kind) throws ParseException {
		...
		if (token.kind == kind) {
			...
			if (++jj_gc > 100) {
				jj_gc = 0;
				for (final JJCalls jj_2Rtn : jj_2_rtns) {
					JJCalls c = jj_2Rtn;
					while (c != null) {
						if (c.gen < jj_gen) {
							c.first = null;
	...
	public ParseException generateParseException() {
		jj_expentries.clear();
		final boolean[] la1tokens = new boolean[103];
		if (jj_kind >= 0) {
