...
public class MediaWikiReferenceParser extends WikiReferenceParser {
	...
	public WikiReference parse(String str) {
		...
		if (str.contains("|") && !str.endsWith("|")) {
			...
			final String label = str.trim().substring(str.indexOf('|') + 1);
			final int pipeCount = str.replaceAll("[^|]", "").length();
			if (reference.toLowerCase().startsWith("file:") && pipeCount > 1 || reference.toLowerCase().startsWith("image:") && pipeCount > 0) {
				final WikiParameters params = generateImageParams(label);
		...
		// Case: [http://mediawiki.org MediaWiki]
			...
			final String link = str.substring(0, str.indexOf(' ')).trim();
			final String label = str.substring(str.indexOf(' ') + 1).trim();
	...
	/**
	 * Generate WikiParameters from the image parameters string this is implemented with the Syntax given at
	 * http://www.mediawiki.org/wiki/Help:Images#Syntax
	 * 
	 * @param paramString
	 *            MediaWiki image parameters as String
	 * @return the WikiParameters.
	 */
	private WikiParameters generateImageParams(String paramString) {
		final List<WikiParameter> paramsList = new ArrayList<WikiParameter>();
