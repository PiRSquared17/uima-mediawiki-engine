...
/**
 * This class is used as a common parser of references. It is used to transform references found in wiki documents into
 * corresponding structured objects - {@link WikiReference}. Methods of this class should be overloaded to parse
...
public class WikiReferenceParser implements IWikiReferenceParser {
	...
	protected WikiParameters getParameters(String[] chunks) {
		return WikiParameters.newWikiParameters(chunks.length > 2 ? chunks[2].trim() : null);
	}
	...
	public WikiReference parse(String str) {
		...
		final String[] chunks = splitToChunks(str);
		if (chunks.length == 0)
			return null;

		final String link = getLink(chunks);
		final String label = getLabel(chunks);
		final WikiParameters parameters = getParameters(chunks);
	...
	protected String[] splitToChunks(String str) {
		...
		final String[] chunks = str.split(delimiter);
		return chunks;
	}
