...
public class WikiScannerUtil {
	...
	/**
	 * Extracts and returns a substring of the given string starting from the given open sequence and finishing by the
	 * specified close sequence. This method unescapes all symbols prefixed by the given escape symbol.
	 * 
	 * @param str
	 *            from this string the substring framed by the specified open and close sequence will be returned
	 * @param open
	 *            the start substring sequence
	 * @param close
	 *            the closing substring sequence
	 * @return a substring of the given string starting from the given open sequence and finishing by the specified
	 *         close sequence
	 */
	 ...
	 *            the escape symbol
	 * @param cleanEscape
	 *            indicate if the escaping char has to be removed. Useful when the substring use the same escaping that
	 *            the string.
	 * @return a substring of the given string starting from the given open sequence and finishing by the specified
	 *         close sequence
	 */
	...
	/**
	 * This method copies to the given buffer all characters from the specified position of the character array to the
	 * next delimiter position. This method returns the next position just after the delimiter (or the length of the
	 * array if no delimiters was found).
	 * 
	 * @param array
	 *            the array of character used as a source of chars
	 * @param pos
	 *            the start position in the array
	 * @param delimiter
	 *            the delimiter; this method copies all character from the current position to the first delimiter
	 *            sequence
	 * @param buf
	 *            the buffer where the content should be appended
	 * @return the next position just after the delimiter of the end of the sequence
	 */
		...
		for (; pos < array.length; pos++) {
			...
			if (quot != 0) {
				...
				if (ch == '\"') {
					...
					// current position
					final int i = skipSequence(array, pos, delimiter);
					if (i > pos) {
						pos = i;
						break;
					}
	...
	/**
	 * @param array
	 *            from this array of bytes the next token will be returned
	 * @param pos
	 *            the current position in the array of bytes
	 * @param buf
	 *            to this buffer the extracted token value will be appended
	 * @param trim
	 *            this array is used to return the boolean flag specifying if the value collected in the buffer should
	 *            be trimmed or not
	 * @param escapeChar
		...
		if (pos < array.length && (array[pos] == '\'' || array[pos] == '"')) {
			...
			for (; pos < array.length; pos++) {
				if (escaped) {
					...
					if (array[pos] == '=' || skipSequence(array, pos, delimiter) > pos) {
						break;
					}
	 ...
	 * @param arrayPos
	 *            the position of the first character in the array; starting from this position the sequence should be
	 *            skipped
	 * @param sequence
	 ...
	 * 
	 * @param array
	 *            the array of characters where the spaces are searched
	 * @param pos
	 *            the current position in the array; starting from this position the spaces will be searched
	 * @param buf
	 *            to this buffer all not empty characters will be added
	 * @return the new position int the array of characters
	 */
	private static int removeSpaces(char[] array, int pos, StringBuffer buf) {
		buf.delete(0, buf.length());
		for (; pos < array.length && (array[pos] == '=' || Character.isSpaceChar(array[pos])); pos++) {
			if (array[pos] == '=') {
				buf.append(array[pos]);
			}
		}
	 ...
	 *            skipped
	 * @param sequence
	 *            the sequence of characters to skip
	 ...
	 *            the string to split
	 * @param list
	 *            to this list all extracted values will be added
	 */
	 ...
	 *            to this list all extracted values will be added
	 * @param delimiter
	 *            a delimiter for individual key/value pairs
	 */
	 ...
	 *            a delimiter for individual key/value pairs
	 * @param end
	 *            the ending sequence, if null it's not taken into account
	 * @param escapeChar
	 *            the escaping character
	...
	public static int splitToPairs(String str, List<WikiParameter> list, String delimiter, String end, char escapeChar) {
		...
		final char[] delimiterArray = delimiter.toCharArray();
		final char[] endArray = end != null ? end.toCharArray() : new char[0];
		final StringBuffer buf = new StringBuffer();
		int i = 0;
		...
		for (; i < array.length;) {
			...
			final WikiParameter entry = new WikiParameter(key, value);
			list.add(entry);
		}
	...
	/**
	 * Unescapes the given string and returns the result. This method uses the default escape symbol (see
	 * {@link #DEFAULT_ESCAPECHAR}).
	 * 
	 * @param str
	 ...
	 *            the string to unescape
	 * @param escape
	 *            the symbol used to escape characters
	 * @return an unescaped string
	 */
