...
public class InternalWikiScannerContext implements IWikiScannerContext {
	...
	private final InlineState						fInlineState		= new InlineState();
	...
	private final Stack<Boolean>					fPropertyDocStack	= new Stack<Boolean>();

	private final Stack<String>						fPropertyStack		= new Stack<String>();
	...
	private final SectionBuilder<WikiParameters>	fSectionBuilder;
	...
	public void beginList() {
		if ((fBlockType & IBlockTypes.LIST) != IBlockTypes.LIST) {
			...
			final IListListener listener = new IListListener() {
				private final Stack<WikiParameters>	fListParamsStack	= new Stack<WikiParameters>();
	...
	/**
	 * @see main.impl.IWikiScannerContext#beginQuotLine(int)
	 */
	...
	protected void checkStyleOpened() {
		if (isInTable()) {
			...
			final String verbatimContent = fVerbatimContent;
			final String macroName = fMacroName;
	...
	public void endPropertyBlock() {
		closeBlock();
		final String property = fPropertyStack.pop();
		final boolean doc = fPropertyDocStack.pop();
	...
	private void openFormat() {
		if (!fNewFormat.equals(fFormat)) {
			final WikiFormat newFormat = fNewFormat;
			closeFormat();
			fFormat = newFormat;
