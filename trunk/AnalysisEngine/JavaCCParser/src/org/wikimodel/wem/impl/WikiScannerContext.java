...
public class WikiScannerContext implements IWikiScannerContext {
	...
	public WikiScannerContext(IWemListener listener) {
		fListener = listener;
		fSectionBuilder = new SectionBuilder<WikiParameters>(new SectionListener<WikiParameters>() {

			@Override
			public void beginDocument(IPos<WikiParameters> pos) {
				final WikiParameters params = pos.getData();
			...
			public void beginSection(IPos<WikiParameters> pos) {
				final WikiParameters params = pos.getData();
				final int docLevel = pos.getDocumentLevel();
				final int headerLevel = pos.getHeaderLevel();
				fListener.beginSection(docLevel, headerLevel, params);
			}

			@Override
			public void beginSectionContent(IPos<WikiParameters> pos) {
				fListener.beginSectionContent(pos.getDocumentLevel(), pos.getHeaderLevel(), pos.getData());
			}
			...
			public void endSectionContent(IPos<WikiParameters> pos) {
				fListener.endSectionContent(pos.getDocumentLevel(), pos.getHeaderLevel(), pos.getData());
			}
	...
	public void beginDocument() {
		final InternalWikiScannerContext context = pushContext();
	...
	public IWikiScannerContext getContext() {
		if (!fStack.isEmpty())
			return fStack.peek();
		final InternalWikiScannerContext context = newInternalContext();
		fStack.push(context);
		return context;
	}
	...
	protected InternalWikiScannerContext newInternalContext() {
		final InternalWikiScannerContext context = new InternalWikiScannerContext(fSectionBuilder, fListener);
		return context;
	}
	...
	/**
	 * @see main.impl.WikiScannerContext#onFormat(org.wikimodel.wem.WikiStyle, boolean)
	 */
	...
	/**
	 * @see main.impl.WikiScannerContext#onTableRow(org.wikimodel.wem.WikiParameters)
	 */
	...
	/**
	 * @see main.impl.WikiScannerContext#onVerbatim(java.lang.String, boolean)
	 */
