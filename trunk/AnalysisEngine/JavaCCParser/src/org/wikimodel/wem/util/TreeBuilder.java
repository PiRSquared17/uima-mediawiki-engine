...
public final class TreeBuilder<X extends TreeBuilder.IPos<X>> {
	...
	public interface IPos<X extends IPos<X>> {
		...
		/**
		 * @param pos
		 * @return <code>true</code> if the underlying data in both positions are the same
		 */
	...
	private static <X extends IPos<X>> void addTail(ITreeListener<X> listener, List<X> firstArray, List<X> secondArray, int secondPos, boolean openTree) {
		final X n = getNode(secondArray, secondPos);
		if (n == null)
			return;
		if (openTree) {
	...
	private static <X extends IPos<X>> void doAlign(ITreeListener<X> listener, List<X> firstArray, List<X> secondArray, boolean expand) {
		...
		int f;
		int s;
		final int firstLen = firstArray.size();
		final int secondLen = secondArray.size();
		for (f = 0, s = 0; f < firstLen && s < secondLen; f++) {
			final X first = firstArray.get(f);
			final X second = secondArray.get(s);
			final int firstPos = first.getPos();
			final int secondPos = second.getPos();
	...
	private static <X extends IPos<X>> void removeTail(ITreeListener<X> listener, List<X> array, int pos, boolean closeTree, boolean remove) {
		final X node = getNode(array, pos);
		if (node == null)
			return;
	...
	private final ITreeListener<X>	fListener;
